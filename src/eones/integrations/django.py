...
try:  # pragma: no cover - optional dependency
    from django.db.models import DateTimeField as _DjangoDateTimeField

    _DJANGO_AVAILABLE = True
except ImportError:  # pragma: no cover - django not installed
    _DJANGO_AVAILABLE = False
    _DjangoDateTimeField = None

from eones.core.date import Date

if TYPE_CHECKING:  # pragma: no cover
    from django.db.models import DateTimeField as _Base
else:
    _Base = _DjangoDateTimeField if _DJANGO_AVAILABLE else object


class DateTimeField(_Base):  # pylint: disable=too-many-ancestors
    ...
    When Django is not installed the class is still importable but
